      ...
      <div className="flex justify-around">
        {topDeliveryMen.map((deliveryMan, index) => (
          <div key={index} className="bg-white rounded-lg shadow-lg p-6 max-w-xs">
            ...
            />
            <h3 className="text-xl font-semibold text-center">{deliveryMan.name}</h3>
            <p className="text-center mt-2">
              <span className="font-semibold">Parcels Delivered:</span> {deliveryMan.parcelsDelivered}
            </p>
            <p className="text-center mt-2">
              <span className="font-semibold">Average Rating:</span> {deliveryMan.averageReview} / 5
            </p>
          </div>
        ))}
      </div>
